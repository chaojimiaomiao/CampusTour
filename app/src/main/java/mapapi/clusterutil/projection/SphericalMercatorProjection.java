...
import mapapi.clusterutil.projection.*;
...
public class SphericalMercatorProjection {
    ...
    public mapapi.clusterutil.projection.Point toPoint(final LatLng latLng) {
        final double x = latLng.longitude / 360 + .5;
        final double siny = Math.sin(Math.toRadians(latLng.latitude));
        final double y = 0.5 * Math.log((1 + siny) / (1 - siny)) / -(2 * Math.PI) + .5;

        return new mapapi.clusterutil.projection.Point(x * mWorldWidth, y * mWorldWidth);
    }

    public LatLng toLatLng(mapapi.clusterutil.projection.Point point) {
